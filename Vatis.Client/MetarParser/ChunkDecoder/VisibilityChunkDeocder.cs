    ...
    public sealed class VisibilityChunkDecoder : MetarChunkDecoder
    {
        ...
        public override Dictionary<string, object> Parse(string remainingMetar, bool withCavok = false)
        {
            ...
            else
            {
                ...
                {
                    // icao visibility
                    visibility.PrevailingVisibility = new Value(Convert.ToDouble(found[2].Value), Value.Unit.M);
                    if (!string.IsNullOrEmpty(found[4].Value))
                    {
                        visibility.MinimumVisibility = new Value(Convert.ToDouble(found[5].Value), Value.Unit.M);
                ...
                else
                {
                    ...
                    visibility.RawValue = found[7].Value.Trim();

                    // whole value
                    if (!string.IsNullOrEmpty(found[8].Value))
                    {
                        visibilityValue = Convert.ToInt32(found[8].Value);
                    }
