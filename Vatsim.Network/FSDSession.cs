...
namespace Vatsim.Network
{
	public class FSDSession
	{
		...
		public static extern IntPtr GenerateAuthResponse(string challengeKey, string key, string clientHash);
		...
		public event EventHandler<DataReceivedEventArgs<PDUSendFastPositions>> SendFastPositionsReceived;
		public event EventHandler<DataReceivedEventArgs<PDUChangeServer>> ChangeServerReceived;
		...
		{
			if (FastPilotPositionReceived != null)
			{
				if (mSyncContext != null)
				{
					mSyncContext.Post((o) =>
		...
		{
			if (SendFastPositionsReceived != null)
			{
				if (mSyncContext != null)
				{
					mSyncContext.Post((o) =>
		...
		public void SendPDU(PDUBase pdu)
		{
			if (mChallengeServer)
			{
				if ((pdu is PDUClientIdentification) && string.IsNullOrEmpty((pdu as PDUClientIdentification).InitialChallenge))
				{
					...
					mServerAuthSessionKey = Marshal.PtrToStringAnsi(GenerateAuthResponse(initialChallenge, null, ClientProperties.ClientHash));
		...
		private void CheckServerAuthChallengeResponse(string response)
		{
			...
			string expectedResponse = Marshal.PtrToStringAnsi(GenerateAuthResponse(mLastServerAuthChallenge, mServerAuthChallengeKey, ClientProperties.ClientHash));
							...
							switch (pduTypeId)
							{
								case "$DI":
									{
										...
										if (GetClientKey() != 0)
										{
											mClientAuthSessionKey = Marshal.PtrToStringAnsi(GenerateAuthResponse(pdu.InitialChallengeKey, null, ClientProperties.ClientHash));
								...
								case "$ZC":
									if (GetClientKey() != 0)
									{
										...
										string response = Marshal.PtrToStringAnsi(GenerateAuthResponse(pdu.Challenge, mClientAuthChallengeKey, ClientProperties.ClientHash));
